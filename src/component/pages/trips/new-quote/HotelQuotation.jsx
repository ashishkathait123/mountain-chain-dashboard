...
const HotelQuotation = ({ 
    ...
    useEffect(() => {
        ...
        if (queryData?.startDate && queryData?.numberOfNights) {
            ...
            const dates = Array.from({ length: queryData.numberOfNights }, (_, i) => { 
    ...
    const accommodationTotal = useMemo(() => {
        const hotelSelling = hotelEntries.reduce((acc, entry) => acc + ((Number(entry.sellingPrice) || 0) * (entry.stayNights?.length || 0)), 0);
    ...
    const hotelOptions = useMemo(() => {
        if (!queryData?.destination?.name || allHotels.length === 0) return [];
        const destName = queryData.destination.name.toLowerCase();
        return allHotels
            .filter(h => h.city?.toLowerCase() === destName)
            .map(h => ({ value: h, label: h.name }));
    }, [allHotels, queryData?.destination?.name]);

    const inclusionHotelOptions = useMemo(() => 
        hotelEntries.filter(e => e.selectedHotel).map(e => ({ value: e.id, label: e.selectedHotel.name })), 
        [hotelEntries]);
    
    const getNightOptionsForHotel = (hotelEntryId) => {
        const entry = hotelEntries.find(e => e.id === hotelEntryId);
        if (!entry || !entry.stayNights) return [];
        return entry.stayNights.map(i => ({ 
            value: i, 
            label: `${i + 1}N (${tripDates[i]?.toLocaleDateString('en-US', { weekday: 'short', day: '2-digit', month: 'short' }) || 'N/A'})` 
        }));
    };
    ...
    const calculateNetRate = (entry) => { 
        if (!entry.selectedRoomConfig) return 0; 
            ...
            <div className="max-w-screen-xl mx-auto">
                <header className="flex items-center space-x-4 mb-6">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500">Add hotel details for <span className='font-bold'>{queryData?.destination?.name}</span>.</p>
                        <p className="text-xs text-slate-400 mt-1">💡 Tip: Use <span className="font-semibold">Duplicate</span> to quickly add similar hotel stays.</p>
                    </div>
                </header>
                <div className="space-y-6">
                    {hotelEntries.map((entry) => {
                        ...
                        // Create options for the night dropdown
                        const nightOptions = tripDates.map((d, i) => ({
                            value: i, 
                            label: `${i + 1}N (${d.toLocaleDateString('en-US', { day: '2-digit', month: 'short' })})` 
                        }));
                        
                        // Create the current value for the night dropdown
                        const nightValue = entry.stayNights?.map(n => nightOptions.find(opt => opt.value === n)) || [];
                        ...
                        return (
                            <div key={entry.id} className="bg-white rounded-lg shadow-inner border border-slate-200">
                                    ...
                                    <div className="xl:col-span-2">
                                        <div className="grid grid-cols-1 md:grid-cols-4 gap-x-4 gap-y-4">
                                            <div>
                                                ...
                                                <Select 
                                                    ...
                                                    value={nightValue} 
                                                    onChange={opts => onUpdateEntry(entry.id, 'stayNights', opts.map(o => o.value))} 
                                                    styles={selectStyles} 
                                            ...
                                            <div>
                                                ...
                                                <Select 
                                                    ...
                                                    value={entry.selectedHotel ? { value: entry.selectedHotel, label: entry.selectedHotel.name } : null} 
                                                    onChange={opt => onUpdateEntry(entry.id, 'selectedHotel', opt.value)} 
                                                    styles={selectStyles} 
                                                    placeholder="Type to search..." 
                                                    isLoading={isLoading} 
                                                    noOptionsMessage={() => 'No hotels found.'}
                                                />
                                                {entry.selectedHotel && <p className="text-xs text-slate-500 mt-1">{entry.selectedHotel.city} • {entry.selectedHotel.stars} Star</p>}
                                            </div>
                                            <div>
                                                <label className="block text-xs font-semibold text-slate-600 mb-1">Meal Plan</label>
                                                <Select 
                                                    options={getMealOptions(entry.selectedHotel)} 
                                                    value={entry.mealPlan ? { value: entry.mealPlan, label: entry.mealPlan } : null} 
                                                    onChange={opt => onUpdateEntry(entry.id, 'mealPlan', opt.value)} 
                                                    isDisabled={!entry.selectedHotel} 
                                                    styles={selectStyles} 
                                                    placeholder="Select..."
                                                />
                                            </div>
                                            <div>
                                                ...
                                                <Select 
                                                    options={getRoomOptions(entry.selectedHotel)} 
                                                    value={entry.selectedRoomConfig ? { value: entry.selectedRoomConfig, label: entry.selectedRoomConfig.roomTypes?.join(', ') } : null} 
                                                    onChange={opt => onUpdateEntry(entry.id, 'selectedRoomConfig', opt.value)} 
                                                    isDisabled={!entry.selectedHotel} 
                                                    styles={selectStyles} 
                                                    placeholder="Select..."
                                                />
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-x-4 gap-y-4 mt-4 pt-3 border-t border-slate-100">
                                            <InputField 
                                                ...
                                                onChange={e => onUpdateEntry(entry.id, 'paxPerRoom', parseInt(e.target.value) || 2)} 
                                                disabled={!entry.selectedRoomConfig} 
                                            />
                                            <InputField 
                                                label="No. of rooms" 
                                                type="number" 
                                                ...
                                                onChange={e => onUpdateEntry(entry.id, 'numRooms', Math.max(1, parseInt(e.target.value) || 1))} 
                                                disabled={!entry.selectedRoomConfig} 
                                            />
                                            <InputField 
                                                label={`AWEB (${formatCurrency(entry.selectedRoomConfig?.AWEB)})`} 
                                                type="number" 
                                                min="0" 
                                                value={entry.numAWEB} 
                                                onChange={e => onUpdateEntry(entry.id, 'numAWEB', parseInt(e.target.value) || 0)} 
                                                disabled={!entry.selectedRoomConfig} 
                                            />
                                            <InputField 
                                                label={`CWEB (${formatCurrency(entry.selectedRoomConfig?.CWEB)})`} 
                                                type="number" 
                                                min="0" 
                                                value={entry.numCWEB} 
                                                onChange={e => onUpdateEntry(entry.id, 'numCWEB', parseInt(e.target.value) || 0)} 
                                                disabled={!entry.selectedRoomConfig} 
                                            />
                                            <InputField 
                                                label={`CNB (${formatCurrency(entry.selectedRoomConfig?.CWoEB)})`} 
                                                type="number" 
                                                min="0" 
                                                value={entry.numCNB} 
                                                onChange={e => onUpdateEntry(entry.id, 'numCNB', parseInt(e.target.value) || 0)} 
                                                disabled={!entry.selectedRoomConfig} 
                                            />
                                            <div>
                                                    ...
                                                    <div key={nightIndex} className="flex justify-between items-center text-sm">
                                                        <div>
                                                            <p className="font-semibold text-slate-800">
                                                                {tripDates[nightIndex]?.toLocaleDateString('en-US', { day: 'numeric', month: 'short' })}
                                                            </p>
                                                            <p className="text-xs text-slate-500">
                                                                {tripDates[nightIndex]?.toLocaleDateString('en-US', { weekday: 'long' })}
                                                            </p>
                                                        </div>
                                                        ...
                                                        <div className="relative">
                                                            ...
                                                            <input 
                                                                ...
                                                                onChange={e => onUpdateEntry(entry.id, 'sellingPrice', parseInt(e.target.value) || 0)} 
                                                                className={`font-bold rounded-md h-9 w-28 text-right pr-2 pl-9 text-sm border ${priceColorClass}`} 
                                                            />
                                                            {sellingPrice < netRate && sellingPrice > 0 && (
                                                                <FaExclamationTriangle 
                                                                    className="absolute right-2 top-1/2 -translate-y-1/2 text-red-500" 
                                                                    title="Selling price is lower than net rate!"
                                                                />
                                                            )}
                                                        </div>
                                                    </div>
                                ...
                                <div className="flex items-center space-x-4 mt-3 p-4 border-t border-slate-100 bg-slate-50/70 rounded-b-lg">
                                    <button 
                                    ...
                                    {hotelEntries.length > 1 && (
                                        <button 
                                            ...
                                            className="flex items-center text-xs font-semibold text-red-500 hover:text-red-700"
                                        >
                ...
                <div className="mt-8">
                    <button 
                        onClick={onAddEntry} 
                        className="w-full md:w-auto flex items-center justify-center bg-blue-100 text-blue-700 font-bold py-2 px-4 rounded-lg hover:bg-blue-200 transition-colors"
                    >
                        <FiPlus className="mr-2"/>Add Hotel Stay
                    </button>
                </div>
                <div className="mt-8 pt-6 border-t border-slate-200">
                    ...
                    <div className="space-y-3">
                        {specialInclusions.map((inclusion, index) => (
                            <div key={inclusion.id} className="grid grid-cols-12 gap-x-4 gap-y-2 items-center">
                                <div className="col-span-12 sm:col-span-3">
                                    {index === 0 && <label className="block text-xs font-semibold text-slate-600 mb-1">Service</label>}
                                    <input 
                                        type="text" 
                                        placeholder="e.g. Honeymoon Cake" 
                                        value={inclusion.service} 
                                        onChange={e => onUpdateInclusion(inclusion.id, 'service', e.target.value)} 
                                        className="w-full text-sm border-slate-300 rounded-md"
                                    />
                                </div>
                                <div className="col-span-12 sm:col-span-2">
                                    {index === 0 && <label className="block text-xs font-semibold text-slate-600 mb-1">Hotel</label>}
                                    <Select 
                                        options={inclusionHotelOptions} 
                                        value={inclusionHotelOptions.find(opt => opt.value === inclusion.hotelEntryId)} 
                                        onChange={opt => onUpdateInclusion(inclusion.id, 'hotelEntryId', opt.value)} 
                                        styles={selectStyles} 
                                        placeholder="Select..." 
                                        noOptionsMessage={() => 'Add a hotel first'}
                                    />
                                </div>
                                <div className="col-span-12 sm:col-span-2">
                                    {index === 0 && <label className="block text-xs font-semibold text-slate-600 mb-1">Night</label>}
                                    <Select 
                                        options={getNightOptionsForHotel(inclusion.hotelEntryId)} 
                                        value={getNightOptionsForHotel(inclusion.hotelEntryId).find(opt => opt.value === inclusion.night)} 
                                        onChange={opt => onUpdateInclusion(inclusion.id, 'night', opt.value)} 
                                        styles={selectStyles} 
                                        placeholder="Select..." 
                                        isDisabled={!inclusion.hotelEntryId} 
                                        noOptionsMessage={() => 'Select nights for hotel'}
                                    />
                                </div>
                                <div className="col-span-6 sm:col-span-2">
                                    {index === 0 && <label className="block text-xs font-semibold text-slate-600 mb-1">Total Price (INR)</label>}
                                    <input 
                                        type="number" 
                                        placeholder="e.g. 1200" 
                                        value={inclusion.price} 
                                        onChange={e => onUpdateInclusion(inclusion.id, 'price', parseInt(e.target.value) || 0)} 
                                        className="w-full text-sm border-slate-300 rounded-md"
                                    />
                                </div>
                                <div className="col-span-6 sm:col-span-2">
                                    {index === 0 && <label className="block text-xs font-semibold text-slate-600 mb-1">Comments</label>}
                                    <input 
                                        type="text" 
                                        placeholder="Any comments" 
                                        value={inclusion.comments} 
                                        onChange={e => onUpdateInclusion(inclusion.id, 'comments', e.target.value)} 
                                        className="w-full text-sm border-slate-300 rounded-md"
                                    />
                                </div>
                                <div className="col-span-12 sm:col-span-1 flex items-end justify-start sm:justify-center">
                                    <button 
                                        onClick={() => onRemoveInclusion(inclusion.id)} 
