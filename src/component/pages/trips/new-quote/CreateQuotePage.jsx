...
const CreateQuotePage = () => {
    ...
    const [flights, setFlights] = useState([{ id: 1, from: '', to: query?.destination?._id || '', departureTime: '', arrivalTime: '', airline: '', flightClass: 'Economy', flightNumber: '', adults: query?.noOfAdults || 1, children: query?.childrenAges?.length || 0, infants: 0, costPrice: '', givenPrice: '', comments: '' }]);
    const [hotelEntries, setHotelEntries] = useState([{ id: 1, stayNights: [], selectedHotel: null, selectedRoomConfig: null, mealPlan: null, numRooms: 1, paxPerRoom: query?.noOfAdults || 2, numAWEB: 0, numCWEB: query?.childrenAges?.length || 0, numCNB: 0, sellingPrice: 0, }]);
    const [specialInclusions, setSpecialInclusions] = useState([]);
    const [dayEntries, setDayEntries] = useState([{ id: 1, selectedDays: [], selectedTransportRoute: null, serviceType: null, transportItems: [{ id: 1, selectedCab: null, qty: 1, rate: 0, given: '' }], }]);
    ...
    const [inclusionsExclusions, setInclusionsExclusions] = useState([{ id: 1, category: 'Inclusions', included: '', excluded: 'Anything not in inclusions is excluded' }]);
    const [termsAndConditions, setTermsAndConditions] = useState('');


    // --- HANDLERS (No changes needed here) ---
    const updateFlight = (id, field, value) => setFlights(p => p.map(f => f.id === id ? { ...f, [field]: value } : f));
    const addFlight = () => setFlights(p => [...p, { id: (p[p.length - 1]?.id || 0) + 1, from: '', to: query?.destination?._id || '', departureTime: '', arrivalTime: '', airline: '', flightClass: 'Economy', flightNumber: '', adults: query?.noOfAdults || 1, children: query?.childrenAges?.length || 0, infants: 0, costPrice: '', givenPrice: '', comments: '' }]);
    ...
    const addNextDayService = (id) => { /* ...logic... */ };
    ...
    const hotelTotals = useMemo(() => { const entriesTotal = hotelEntries.reduce((acc, h) => ({ net: acc.net, selling: acc.selling + ((Number(h.sellingPrice) || 0) * (h.stayNights?.length || 1)) }), { net: 0, selling: 0 }); const inclusionsTotal = specialInclusions.reduce((acc, i) => acc + (Number(i.price) || 0), 0); return { net: entriesTotal.net + inclusionsTotal, selling: entriesTotal.selling + inclusionsTotal, entries: hotelEntries, inclusions: specialInclusions }; }, [hotelEntries, specialInclusions]);
    const transportTotals = useMemo(() => { const entriesTotal = dayEntries.reduce((acc, d) => { const numDays = d.selectedDays.length || 1; d.transportItems.forEach(i => { acc.selling += Number(i.given || 0) * i.qty * numDays; acc.net += Number(i.rate || 0) * i.qty * numDays; }); return acc; }, { net: 0, selling: 0 }); const extrasTotal = transportExtraServices.reduce((acc, s) => acc + (Number(s.price) || 0), 0); return { net: entriesTotal.net + extrasTotal, selling: entriesTotal.selling + extrasTotal }; }, [dayEntries, transportExtraServices]);

    const handleSaveQuote = async (summaryData) => {
        setIsSaving(true);
        const token = sessionStorage.getItem('token');
        if (!token) { toast.error("Authentication error. Please log in again."); setIsSaving(false); return; }

        const formattedPayload = {
            queryId: query._id,
            startDate: query.startDate,
            duration: query.duration,
            pax: query.noOfAdults,
            flights: flights.map(({ id, ...rest }) => rest),
            hotelDetails: {
                entries: hotelEntries.map(({ id, selectedHotel, selectedRoomConfig, ...rest }) => ({
                    ...rest,
                    hotelId: selectedHotel?._id,
                    hotelName: selectedHotel?.name,
                    roomConfig: selectedRoomConfig?.label
                })),
                specialInclusions: specialInclusions.map(({ id, hotelEntryId, ...rest }) => {
                    const hotelEntry = hotelEntries.find(h => h.id === hotelEntryId);
                    return { ...rest, hotelName: hotelEntry?.selectedHotel?.name || 'N/A' };
                }),
            },
            transportDetails: {
                entries: dayEntries.map(({ id, selectedTransportRoute, transportItems, ...rest }) => ({
                    ...rest,
                    serviceName: selectedTransportRoute?.serviceName,
                    route: selectedTransportRoute ? `${selectedTransportRoute.fromCity} to ${selectedTransportRoute.toCity}` : '',
                    transportItems: transportItems.map(({ id, selectedCab, given, ...itemRest }) => ({
                        ...itemRest,
                        givenPrice: given,
                        cabDetails: selectedCab ? `${selectedCab.cabName} (${selectedCab.cabType})` : ''
                    }))
                })),
                extraServices: transportExtraServices.map(({ id, ...rest }) => rest),
            },
            dayWiseItinerary: dayWiseItinerary,
            inclusionsExclusions: inclusionsExclusions.map(({ id, ...rest }) => rest),
            termsAndConditions: termsAndConditions.split('\n').filter(line => line.trim() !== ''),
            
            // **** FIX #2: Match the summary structure from your log ****
            summary: {
                ...summaryData, // This will include markup, gst, rounding, etc.
                totalNetCost: flightTotals.net + hotelTotals.net + transportTotals.net,
                totalSellingPrice: flightTotals.selling + hotelTotals.selling + transportTotals.selling,
            }
        };

        console.log("Payload to be sent:", JSON.stringify(formattedPayload, null, 2));

        try {
            await axios.post(
                `${API_BASE_URL}/quotations`, // This now correctly uses the defined constant
                formattedPayload,
    ...
    if (!query) return (<div>Error: No Query Data Found. Please go back and select a trip.</div>);
            ...
            <div className="max-w-screen-xl mx-auto space-y-8">
                <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
                    ...
                    <div className="flex flex-wrap items-center gap-x-10 gap-y-4 text-sm">
                        <div><p className="font-medium text-base">{query.guestName}</p></div>
                ...
                <HotelQuotation queryData={query} hotelEntries={hotelEntries} specialInclusions={specialInclusions} onUpdateEntry={updateHotelEntry} onAddEntry={addHotelEntry} onRemoveEntry={removeHotelEntry} onDuplicateEntry={duplicateHotelEntry} onUpdateInclusion={updateInclusion} onAddInclusion={addInclusion} onRemoveInclusion={removeInclusion} />
                <TransportQuotation queryData={query} dayEntries={dayEntries} extraServices={transportExtraServices} sameCabForAll={sameCabForAll} onUpdateDayEntry={updateDayEntry} onAddDayEntry={addDayEntry} onRemoveDayEntry={removeDayEntry} onDuplicateDayEntry={duplicateDayEntry} onAddNextDayService={addNextDayService} onUpdateTransportItem={updateTransportItem} onAddTransportItem={addTransportItem} onRemoveTransportItem={removeTransportItem} onHandleDaySelection={handleDaySelection} onUpdateExtraService={updateExtraTransportService} onAddExtraService={addExtraTransportService} onRemoveExtraService={removeExtraTransportService} onSetSameCabForAll={setSameCabForAll} />
                
                <ItineraryDetails 
                    queryData={query}
                    itinerary={dayWiseItinerary}
                    setItinerary={setDayWiseItinerary}
                    inclusions={inclusionsExclusions}
                    setInclusions={setInclusionsExclusions}
                    terms={termsAndConditions}
                    setTerms={setTermsAndConditions}
                />
                
                <Summary queryData={query} flightData={flightTotals} hotelData={hotelTotals} transportData={transportTotals} onSave={handleSaveQuote} isSaving={isSaving} />
            </div>
        </div>
    );
};
