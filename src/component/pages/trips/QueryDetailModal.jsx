...
import ServicesBookingsTab from '../../../services/bookingServices';
import AccountingTab from '../accounting/Accounting';
import DocsTab from '../accounting/Invoices';
...
const convertedQuote = useMemo(() => {
        if (editedQuery?.status !== 'Converted' || !quotations.length) return null;
        // First check if the populated data is available
        if (editedQuery.convertedWithQuote && typeof editedQuery.convertedWithQuote === 'object') {
            return editedQuery.convertedWithQuote;
        }
        // Fallback to searching in the quotations list
        return quotations.find(q => q._id === editedQuery.convertedWithQuote);
    }, [editedQuery, quotations]);
   ...
   useEffect(() => {
        const fetchAllData = async () => {
            ...
            try {
                // We now need to populate the converted quote details if they exist
                const queryRes = await axios.get(`${API_BASE_URL}/destination/getquery/${id}`, { 
                    headers: { Authorization: `Bearer ${token}` } 
                });
                
                // ✅ POPULATE the converted quote
                 const populatedQueryRes = await axios.get(`${API_BASE_URL}/destination/getquery/${id}?populate=convertedWithQuote`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                const queryData = populatedQueryRes.data;

                setEditedQuery(queryData);

                const [quotesRes, followUpsRes] = await Promise.all([
                    axios.get(`${API_BASE_URL}/quotations/by-query/${queryData._id}`, { headers: { Authorization: `Bearer ${token}` } }),
                    axios.get(`${API_BASE_URL}/destination/getfollowupsquery/${queryData._id}`, { headers: { Authorization: `Bearer ${token}` } })
                ]);
                
                setQuotations(quotesRes.data || []);
                setFollowUps(followUpsRes.data.data || []);

                // ✅ Set the default tab based on status
                if (queryData.status === 'Converted') {
                    setActiveTab('Services Bookings');
                } else {
 ...
 const isConverted = editedQuery?.status === 'Converted';
    const PRE_CONVERSION_TABS = ['Basic Details', 'All Quotes', 'New Quote'];
    const POST_CONVERSION_TABS = ['Basic Details', 'All Quotes', 'Services Bookings', 'Accounting', 'Docs', 'Activities'];
    const tabsToShow = isConverted ? POST_CONVERSION_TABS : PRE_CONVERSION_TABS;

    if (loading) return <div className="flex justify-center items-center h-screen">Loading...</div>;
    if (!editedQuery) return <div className="p-4 text-center text-red-500">Query not found.</div>;

   return (
    <>
        {/* --- ADD FOLLOW-UP MODAL (No changes here) --- */}
        {isModalOpen && (
        ...
        <div className="p-4 sm:p-6 lg:p-8 bg-slate-50 min-h-screen text-slate-800">
            <div className="max-w-screen-2xl mx-auto">
                ...
                <header className="mb-0 bg-white p-4 rounded-t-lg shadow-sm border">
                    <div className="flex justify-between items-start">
                        ...
                        <div className="flex items-center space-x-2">
                            {/* ✅ UPDATED: DYNAMIC STATUS BADGE COLOR */}
                            <div className={`text-xs font-semibold text-white px-2 py-1 rounded ${isConverted ? 'bg-green-600' : 'bg-blue-500'}`}>
                                {editedQuery.status}
                            </div>
                ...
                {/* ✅ UPDATED: DYNAMIC TAB NAVIGATION */}
                ...
                {/* ✅ UPDATED: DYNAMIC MAIN CONTENT */}
                <main className="grid grid-cols-12 gap-6 mt-6">
                    <div className="col-span-12 lg:col-span-9">
                        <div className="bg-white rounded-lg shadow-sm border min-h-[600px]">
                            {activeTab === 'Basic Details' && (
                                <QuotationDetails quote={isConverted ? convertedQuote : latestQuote} convertedQuoteId={editedQuery.convertedWithQuote} />
                            )}
                            
                            {activeTab === 'All Quotes' && (
                                <AllQuotesTab 
                                    quotations={quotations} 
                                    onUseQuote={(quoteId) => navigate(`/quotes/${quoteId}`)} 
                                />
                            )}
                            
                            {/* --- PRE-CONVERSION VIEW --- */}
                            {!isConverted && activeTab === 'New Quote' && (
                                <NewQuoteTab 
                                    onNavigateToCreate={() => navigate("/new-quote", { state: { query: editedQuery } })} 
                                />
                            )}
                            
                            {/* --- POST-CONVERSION VIEW --- */}
                            {isConverted && activeTab === 'Services Bookings' && (
                                <ServicesBookingsTab query={editedQuery} quote={convertedQuote} />
                            )}
                             {isConverted && activeTab === 'Accounting' && (
                                <AccountingTab query={editedQuery} quote={convertedQuote} />
                            )}
                             {isConverted && activeTab === 'Docs' && (
                                <DocsTab query={editedQuery} />
                            )}

                            {/* --- UNIVERSAL VIEW --- */}
                             {activeTab === 'Activities' && (
                                <ActivitiesTab query={editedQuery} />
                            )}
                        </div>
                    </div>
                    
                    {/* Right Column (Follow-ups) - No changes here */}
