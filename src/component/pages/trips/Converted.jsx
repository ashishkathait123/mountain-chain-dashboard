...
const Converted = () => {
  ...
  useEffect(() => {
    ...
    const fetchTrips = async () => {
      ...
      try {
        ...
        // Filter the data to only include trips with "Converted" status
        const convertedTrips = response.data.data.filter(query => query.status === "Converted");
        setTrips(convertedTrips);

      } catch (error) {
        console.error('Error fetching converted trips:', error);
        toast.error('Failed to fetch converted trips.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return <div className="p-4 text-center text-slate-500">Loading Converted Trips...</div>;
  }
      ...
      <div className="bg-white rounded-lg shadow overflow-x-auto">
        <div className="p-4 border-b">
          <h1 className="text-base sm:text-lg text-black font-semibold">
            Showing 1 - {trips.length} of {trips.length} Converted Trips
          </h1>
        </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {trips.length > 0 ? trips.map((query) => (
                <tr key={query._id} className="hover:bg-gray-50">
                  ...
                  <td className="w-1/6 px-4 py-3">
                    {/* ✅ Changed badge color for Converted status */}
                ...
                <tr>
                    <td colSpan="5" className="text-center p-6 text-gray-500">
                        No trips have been converted yet.
                    </td>
                </tr>
              )}
            </tbody>
